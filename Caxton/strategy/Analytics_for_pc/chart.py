    ...
    for i in range(df_param.shape[0]):
        ...
        if this_name.split('_')[0] != 'zzz':
            ...
            try:
                ...
                _chart_rate_rise_fall_file[this_name] = _chart_rate_rise_fall_file[this_name] + [
                    os.path.join(PROJ_DIR, rise_fall)]
            except:
                pass
...
def temp_charts(_component_name_list, _comp_chart, _comp_chart_title, _comp_chart_n, _comp_chart_color,
                _comp_chart_style, _comp_chart_twox, _comp_chart_ratecycle, _comp_rate_rise_fall_file=None, pdfpath='',
                bt_backup_dir=None, start_dt='2000-01-01', end_dt=datetime.now().strftime('%Y-%m-%d'), page_name='',
    ...
    for i, n in enumerate(chart_in_page):
        ...
        dfs_in_this_page, title_itp, n_itp, color_itp, stype_itp, twox_itp, rate_cycle_itp, rate_rise_fall_path_itp = dfs[
                                                                                                                      start_idx:end_idx], titles[
                                                                                                                                          start_idx:end_idx], number_of_lines[
                                                                                                                                                              start_idx:end_idx], colors[
                                                                                                                                                                                  start_idx:end_idx], styles[
                                                                                                                                                                                                      start_idx:end_idx], two_x[
                                                                                                                                                                                                                          start_idx:end_idx], rate_cycle[
                                                                                                                                                                                                                                              start_idx:end_idx], rate_rise_fall[
                                                                                                                                                                                                                                                                  start_idx:end_idx]
        ...
        for i in range(chart_rows):
            for j in range(chart_cols):
                if i * chart_cols + j < len(df_in_this_page):
                    ...
                    current_dfs, current_title, current_n, current_color, current_style, current_twox, current_rate_cycle, current_rate_rise_path = \
                    dfs_in_this_page[
                        i * chart_cols + j], \
                    title_itp[i * chart_cols + j], \
                    n_itp[i * chart_cols + j], \
                    color_itp[i * chart_cols + j], \
                    stype_itp[i * chart_cols + j], twox_itp[i * chart_cols + j], rate_cycle_itp[i * chart_cols + j], \
                    rate_rise_fall_path_itp[i * chart_cols + j]
                    print(current_n)
                    print(i, j, id)
                        ...
                        if current_twox == 'enable':
                            ...
                            if current_style[1] == 'shaded_with_line':
                                ...
                                line2 = ax2.plot(x2, y2, color=current_color[1], ls=current_style[1], lw=0.9,
                                                 label=df2.columns.tolist()[0])
                        elif (current_twox == 'automatic') and (np.mean(np.abs(
                                y1 - y2)) > 10):  # or np.max(y1/y2)>10 or np.max(y1/y2) < 0.1 or np.min(y1/y2) > 10 or np.min(y1/y2) < 0.1:
                            two_ax = True
                            ax2 = ax.twinx()
                            if current_style[1] == 'shaded_with_line':
                                line2 = ax2.plot(x2, y2, color=current_color[1], ls='solid', lw=0.4,
...
def add_rates_rise_fall(ax, type='type1', rise_dates_path=os.path.join(PROJ_DIR,
                                                                       r"basket\USA_Rates1\usaratesfalling_rising_period.xlsx")):
    try:
...
def rates_tree_component_plot(plot_dict, chart_start_dt='2010-01-01', chart_end_dt='2035-01-01', rate_rise_fall='',
                              pdfpath='', page_name='', bt_backup_dir=None):
    '''
    ...
    for i, n in enumerate(chart_in_page):
        ...
        start_idx = sum(chart_in_page[:i])
        end_idx = start_idx + n
        dfs_in_this_page, title_itp, chart_style_itp, rate_rise_fall_path_itp = df_pair[start_idx:end_idx], title_list[
                                                                                                            start_idx:end_idx], chart_style[
                                                                                                                                start_idx:end_idx], rate_rise_fall[
                                                                                                                                                    start_idx:end_idx]
        ...
        for i in range(chart_rows):
            for j in range(chart_cols):
                if i * chart_cols + j < len(title_itp):
                    ax = axarr[i, j]
                    current_dfs, current_title, current_style, current_rate_rise_path = dfs_in_this_page[
                                                                                            i * chart_cols + j], \
                                                                                        title_itp[i * chart_cols + j], \
                                                                                        chart_style_itp[
                                                                                            i * chart_cols + j], \
                                                                                        rate_rise_fall_path_itp[
                                                                                            i * chart_cols + j]
                    ...
                    if current_style in ['smooth_z_score']:
                        ...
                        add_rates_rise_fall(ax, rise_dates_path=current_rate_rise_path)
                        # add_vertical_line_on_date(ax)
                    if current_style in ['gauge_vs_yield', 'cumprof_vs_yield', 'cumprof_vs_signal', 'drawdown_vs_yield',
                                         'risk_breakdown_vs_yield']:
                        df1, df2 = current_dfs[0].dropna(), current_dfs[1].dropna()
                        mask = (df1.index >= chart_start_dt) & (df1.index <= chart_end_dt)
                        df1 = df1.loc[mask, :]
                        mask = (df2.index >= chart_start_dt) & (df2.index <= chart_end_dt)
                    ...
                    if current_style in ['raw_vs_trend_vs_yield']:
                        ...
                        x1, x2, x3 = pd.to_datetime(df1.index).date, pd.to_datetime(df2.index).date, pd.to_datetime(
                            df3.index).date
...
def generic_plot(plot_dict, chart_start_dt='2010-01-01', chart_end_dt='2035-01-01', pdfpath='', page_name='',
                 bt_backup_dir=None, override_chart_params={}):
    '''
    :param plot_dict: dictionary which defines df pairs, titles,
    :param chart_start_dt:
    :return:
    ...
    for i, n in enumerate(chart_in_page):
        ...
        start_idx = sum(chart_in_page[:i])
        end_idx = start_idx + n
        dfs_in_this_page, title_itp, chart_style_itp = df_pair[start_idx:end_idx], title_list[
                                                                                   start_idx:end_idx], chart_style[
                                                                                                       start_idx:end_idx]
        if 'other_param' in plot_dict.keys():
            ...
            for j in range(chart_cols):
                if i * chart_cols + j < len(title_itp):
                    if chart_each_page < 1.1:
                        ...
                        ax = axarr[i, j]
                    current_dfs, current_title, current_style = dfs_in_this_page[i * chart_cols + j], title_itp[
                        i * chart_cols + j], chart_style_itp[i * chart_cols + j]
                    if 'other_param' in plot_dict.keys():
                        current_other_param = other_param_itp[i * chart_cols + j]

                    if current_style in ['raw_vs_trend', 'pred_vs_orig']:
                        df1, df2 = current_dfs[0].dropna(), current_dfs[1].dropna()
                        mask = (df1.index >= chart_start_dt) & (df1.index <= chart_end_dt)
                        df1 = df1.loc[mask, :]
                        mask = (df2.index >= chart_start_dt) & (df2.index <= chart_end_dt)
                        ...
                        if x1[-1].month > 10:
                            ...
                            ax.set_xlim(datemin, datemax + np.timedelta64(1, 'Y'))
                    if current_style in ['raw_only', 'z_score', 'price_or_volume', 'cumprof_only', 'resid_only']:
                        ...
                        # set the max and min
                        if current_style in ['raw_only', 'price_or_volume', 'cumprof_only']:
                            l1_max = df1.loc[chart_start_dt:].max().values
                            l1_min = df1.loc[chart_start_dt:].min().values
                            l1_diff = l1_max - l1_min
                ...
                # add_vertical_line_on_date(ax)
            if current_style in ['gauge_vs_yield', 'cumprof_vs_yield', 'cumprof_vs_signal', 'drawdown_vs_yield',
                                 'risk_breakdown_vs_yield', 'price_n_volume', 'ca_vs_hf_estimate', 'raw_y_vs_raw_x']:
                df1, df2 = current_dfs[0].dropna(), current_dfs[1].dropna()
                mask = (df1.index >= chart_start_dt) & (df1.index <= chart_end_dt)
                df1 = df1.loc[mask, :]
                mask = (df2.index >= chart_start_dt) & (df2.index <= chart_end_dt)
    ...
    if current_style in ['pred_vs_fitted_with_band']:
        # the outlier chart, with 2 sigma confidence interval as the second and third input
        df1, df2, df3, df4 = current_dfs[0].dropna(), current_dfs[1].dropna(), current_dfs[2].dropna(), current_dfs[
            3].dropna()
        mask = (df1.index >= chart_start_dt) & (df1.index <= chart_end_dt)
        df1 = df1.loc[mask, :]
        mask = (df2.index >= chart_start_dt) & (df2.index <= chart_end_dt)
        ...
        x1, x2, x3, x4 = pd.to_datetime(df1.index).date, pd.to_datetime(df2.index).date, pd.to_datetime(
            df3.index).date, pd.to_datetime(df4.index).date
