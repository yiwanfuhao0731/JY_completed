...
class date_utils_collection(object):
    ...
    def remove_outlier(self,df,frac=0.01,n=3):
        from loess import loess_regr as lf
        assert len(df.columns)<1.01,'only 1 columns can be taken!!'
        df = df.copy().dropna()
        y_ticker = df.columns.tolist()[0]
        Lo_Filter = lf(df, y_ticker, frac)
        df_trend = Lo_Filter[['y_fitted']]
    ...
    def adj_so_that_use_latest_info(self, df, running_mode='research'):
        '''
        in production environment, making sure last z_score used for today contains the latest available information
        this algorithm shift the time series little by little to make sure that the latest information is used, with minimum adjustment(shift) being made
        ...
        if running_mode == 'research':
            ...
            if df.dropna().index[-1]<today:
                ...
                df = df.dropna()
                latest_info = df.dropna().iloc[:,0].values[-1]
                today_info = df.loc[today].values[0]
                while latest_info != today_info:
                    df = df.shift(-1)
                    latest_info = df.dropna().iloc[:, 0].values[-1]
                    today_info = df.loc[today].values[0]
                return df

import_dir = r"d:\yangs\Third_yr_grad_English\relevant_file\workspace_zy\Caxton\strategy\indicator.csv"
...
df = SU.adj_so_that_use_latest_info(df,running_mode='production')
