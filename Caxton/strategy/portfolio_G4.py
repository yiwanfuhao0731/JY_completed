...
class signal3(abs_sig):
    ...
    def run_step1(self):
        ...
        self.re_run_if_necessary(self.re_run_time_thres)
    ...
    def re_run_if_necessary(self, min_limit):
        for basket_name in self.ordered_basket:
            ...
            if 'need_to_re_run' in [v for v in f.values()]:
                ...
                _signal3.initialise_and_run(run_charting=False)
    ...
    def run_post_vol_cap_signals(self):
        ...
        for basket_name in self.ordered_basket:
            ...
            df_comb = pd.concat([df_original_signal, df_rb, df_total_scale_factor, df_duration, df_vol_cap_factor], axis=1)
            df_comb['total_vol_factor_post_vol_cap'] = df_comb.iloc[:, 1] * df_comb.iloc[:, 2] / df_comb.iloc[:, 3].shift(
                1) * df_comb.iloc[:, 4]
            df_comb['signal_before_fund_vol_cap'] = df_comb.iloc[:, 0] * df_comb.iloc[:, 1] * df_comb.iloc[:,
                                                                                              2] / df_comb.iloc[:, 3].shift(
                1)
            ...
            df_comb['DV01_before_fund_vol_cap'] = df_comb['signal_before_fund_vol_cap'] * df_comb.iloc[:, 3].shift(
                1) / 10000
            df_comb['DV01_after_fund_vol_cap'] = df_comb['signal_after_fund_vol_cap'] * df_comb.iloc[:, 3].shift(1) / 10000

            _wf.add_df('total_vol_factor_post_vol_cap', df_comb[['total_vol_factor_post_vol_cap']])
            df_comb[['total_vol_factor_post_vol_cap']].dropna().plot(figsize=(10, 5), grid=True, title=basket_name,
                                                                     color='red')
            plt.show()
            _wf.add_df('signal_before_fund_vol_cap', df_comb[['signal_before_fund_vol_cap']])
            df_comb[['signal_before_fund_vol_cap']].dropna().plot(figsize=(10, 5), grid=True, title=basket_name,
                                                                  color='green')
            plt.show()
            _wf.add_df('signal_after_fund_vol_cap', df_comb[['signal_after_fund_vol_cap']])
            df_comb[['signal_after_fund_vol_cap']].dropna().plot(figsize=(10, 5), grid=True, title=basket_name,
                                                                 color='blue')
            plt.show()
            _wf.add_df('DV01_before_fund_vol_cap', df_comb[['DV01_before_fund_vol_cap']])
            df_comb[['DV01_before_fund_vol_cap']].dropna().plot(figsize=(10, 5), grid=True, title=basket_name,
                                                                color='black')
            plt.show()
            _wf.add_df('DV01_after_fund_vol_cap', df_comb[['DV01_after_fund_vol_cap']])
            df_comb[['DV01_after_fund_vol_cap']].dropna().plot(figsize=(10, 5), grid=True, title=basket_name,
                                                               color='orange')
            plt.show()
    ...
    def run_rb_constant_fund_scale(self, *args, **kwargs):
        ...
        df_matrix['rollVol_un_rescale'] = df_matrix.loc[:, 'un_re_scaled_portfolio_ret'].dropna().rolling(500).std() * np.sqrt(
            252)

        # backward fill to the begin of the portfolio
        df_matrix.loc[df_matrix.index >= first_common_ret_index, 'rollVol_un_rescale'].fillna(method='bfill', inplace=True)
        ...
        df_matrix['fundRB_ScaledRet'] = df_matrix.loc[:,
                                        [basket_name + '_RB_ScaledRet' for basket_name in self.ordered_basket]].sum(axis=1)
    ...
    def post_indicator_genr(self, tc_reduction_mehod='inertia'):
        ...
        signal_dict = post_signal_genr.apply_inertia_filtered(signal_after_fund_vol_cap_group, tc_reduction_method='inertia',inertia_torlerance=[50, 50, 50, 50])
